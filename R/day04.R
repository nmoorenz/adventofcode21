...
f04a <- function(x) {
  ...
  for (j in 1:length(bingo_numbers)) {
    ...
    do_we_have_bingo = check_cards(bingo_cards)
...
check_cards <- function(cards) {
  ...
  if (nrow(row_bingo) > 1) print("this is really bad")
  ...
  if (nrow(col_bingo) > 1) print("this is really bad")
